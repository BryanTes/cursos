...
function hook_add_to_quote() {
  // Validar
  if(!isset($_POST['concepto'], $_POST['tipo'], $_POST['largo'], $_POST['ancho'], $_POST['colorVidrio'], $_POST['colorPvc'], $_POST['precio_unitario'], $_POST['cantidad'])) {
  ...
  $colorVidrio = trim($_POST['colorVidrio'],);
  $colorPvc = trim($_POST['colorPvc'],);
  $price    = (float) str_replace([',','$'], '', $_POST['precio_unitario']);
  $quantity = (int) trim($_POST['cantidad']);
  $subtotal = (float) $price * $quantity * $largo * $ancho;
...
function hook_save_concept() {
  // Validar
  if(!isset($_POST['id_concepto'], $_POST['concepto'], $_POST['tipo'], $_POST['largo'], $_POST['ancho'], $_POST['colorVidrio'], $_POST['coloPvc'], $_POST['precio_unitario'], $_POST['cantidad'])) {
    json_output(json_build(403, null, 'Parametros incompletos.'));
  }
  ...
  $colorVidrio     = trim($_POST['colorVidrio']);
  $colorPvc     = trim($_POST['colorPvc']);
  $price    = (float) str_replace([',','Q'], '', $_POST['precio_unitario']);
  $quantity = (int) trim($_POST['cantidad']);
  $subtotal = (float) $price * $quantity * $largo * $ancho;
  $taxes    = (float) $subtotal * (TAXES_RATE / 100);
