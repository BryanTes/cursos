$('document').ready(() => {
  ...
  function add_to_quote(e) {
    ...
    if(concepto.length < 5) {
      notify('Ingresa un producto válido por favor.', 'danger');
      errors++;
    }

    // Validar el precio
    if(precio < 10) {
      notify('Por favor ingresa un precio mayor a Q10.00', 'danger');
      errors++;
    }
  ...
  function edit_concept(e) {
    ...
    $.ajax({
      ...
      if(res.status === 200) {
        ...
        $('#largo', form_update_concept).val(res.data.largo);
        $('#ancho', form_update_concept).val(res.data.ancho);
        //$('#colorPvc option[value="'+res.data.colorPvc+'"]', form_update_concept).attr('selected', true);
